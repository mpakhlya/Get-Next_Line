...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/03/15 18:21:06 by mpakhlya          #+#    #+#             */
/*   Updated: 2024/03/15 18:21:06 by mpakhlya         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	if (fd < 0 || BUFFER_SIZE <= 0 || read(fd, 0, 0) < 0)
		return (NULL);
	buffer[fd] = lire_ficher(fd, buffer[fd]);
	if (!buffer[fd])
		return (NULL);
	la_ligne = ft_la_ligne(buffer[fd]);
	buffer[fd] = ft_prochain(buffer[fd]);
	return (la_ligne);
}
